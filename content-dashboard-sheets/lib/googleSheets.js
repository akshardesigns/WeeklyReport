...
// Urutan kolom di sheet HARUS sama seperti urutan ini (kolom A - G)
const HEADERS = ['id', 'tglMasuk', 'pilar', 'platform', 'brief', 'status', 'tglSelesai'];
...
function getAuth() {
  ...
  if (!email || !rawKey || !SHEET_ID) {
    throw new Error(
      'Konfigurasi belum lengkap. Pastikan GOOGLE_SERVICE_ACCOUNT_EMAIL, GOOGLE_PRIVATE_KEY, dan GOOGLE_SHEET_ID sudah diisi di .env.local'
    );
  }
